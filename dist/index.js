...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const commander_1 = __nccwpck_require__(8909);
const chalk_1 = __importDefault(__nccwpck_require__(5130));
const fs_1 = __importDefault(__nccwpck_require__(9896));
const path_1 = __importDefault(__nccwpck_require__(6928));
const js_yaml_1 = __nccwpck_require__(4281);
const ajv_1 = __importDefault(__nccwpck_require__(2463));
const normalize_1 = __nccwpck_require__(4240);
...
const validateIntent = async (file) => {
    ...
    try {
        ...
        const normalized = (0, normalize_1.normalizeMarkdown)(content, (y) => (0, js_yaml_1.load)(y));
        if (!normalized.ok) {
            console.error(chalk_1.default.red(`❌ Invalid IntentSpec: ${file}`));
            console.error(chalk_1.default.yellow(`- ${normalized.error}`));
            process.exit(1);
        }
        ...
        if (valid) {
            ...
            validate.errors?.forEach(err => {
                const errorPath = err.instancePath || err.dataPath || 'Root';
                console.error(chalk_1.default.yellow(`- ${errorPath} ${err.message}`));
            });
            process.exit(1);
        }
    }
...
const { Help, stripColor } = __nccwpck_require__(9754);
